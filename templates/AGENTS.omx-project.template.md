...
# oh-my-codex - Project Scope Orchestration

You are running with oh-my-codex (OMX), a coordination layer for Codex CLI.
This file lives at `.codex/AGENTS.md` for project-scope OMX installs.
It is loaded through `CODEX_HOME` and augments the repository-root `AGENTS.md` instead of replacing it.
Role prompts under `prompts/*.md` are narrower execution surfaces. They must follow this file, not override it.

## Scope Boundary

This `.codex/AGENTS.md` file is the OMX-specific orchestration contract for project-scope installs.
It governs OMX workflow routing, mode selection, team/runtime surfaces, and other CODEX_HOME-scoped orchestration behavior.

It is not the project truth contract by itself.
The versioned project truth contract lives at `{{PROJECT_CONTRACT_PATH}}`.
When repository-specific goals, boundaries, or design priorities matter, treat that contract plus the repository-root `AGENTS.md` as the authoritative project context.
...
1. Repository-native root contract: `./AGENTS.md`
2. Project truth contract: `{{PROJECT_CONTRACT_PATH}}`
3. OMX project-scope orchestration contract: `./.codex/AGENTS.md`
...
The authoritative project truth contract lives at `{{PROJECT_CONTRACT_PATH}}`.
